...
class App {
  ...
  _loadMap(position) {
    ...
    // function that lets you select cordinate inside map
    // Handle clicks on map
    this.#map.on('click', this._showForm.bind(this));

    // Loading the marker using local storage
  ...
  _getLocalStorage() {
    ...
    // });
    // converst basic objects to class
    this.revieveClasses(data);
  }

  revieveClasses(workoutData) {
    let revieveWorkout = [];
    let running, cycling;
    ...
    workoutData.forEach(work => {
      if (work.cadence) {
        ...
        revieveWorkout.push(running);
      }

      if (work.elevationGain) {
        cycling = new Cycling(
          work.coords,
          work.distance,
          work.duration,
        ...
        revieveWorkout.push(cycling);
      }
    });

    console.log('review', revieveWorkout);

    this.#workouts = revieveWorkout;
    this.#workouts.forEach(work => {
      this._renderWorkout(work);
    });
  }
